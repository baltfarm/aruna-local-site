...
?>
<div class="container">
<div class="row justify-content-center section-header h2-blue">
						...
						<form id="<?=$FORM_ID?>"
							...
							<? if($arParams['DISPLAY_FIELDS']): ?>
								<? foreach($arParams['DISPLAY_FIELDS'] as $FIELD): ?>
									...
									?>
									<div id="<?=$INPUT_ROW_ID?>" class="<?=$INPUT_ROW_CLASS;?> form-border">
										<? if(!$arParams['HIDE_FIELD_NAME']): ?>
										...
										<div class="<?=$INPUT_CONTROLS_CLASS?>">
											...
											if($arField['TYPE'] == 'TEXTAREA'): ?>
												<div class="uk-form-border"><textarea name="FIELDS[<?=$FIELD?>]"
												          id="<?=$INPUT_ID?>" <?=$INPUT_PLACEHOLDER;?>
												          rows="<?=$arParams['FORM_TEXTAREA_ROWS']?>"
												          class="<?=$INPUT_CLASS;?>"><?=$arResult['FIELDS'][ $FIELD ]?></textarea></div>
							...
							<div id="<?=$FORM_ID?>_ROW_BUTTON" class="<?=$INPUT_ROW_CLASS?> group-button">
								...
								<? endif; ?>
								<div class="<?=$INPUT_CONTROLS_CLASS?> text-center">
									<button id="API_FEX_FORM_SUBMIT_<?=$FORM_ID?>"
									        type="submit"
									        name="API_FEX_SUBMIT_BUTTON"
									        class="<?=$arParams['FORM_SUBMIT_CLASS'];?><?=$BUTTON_SIZE?> btn-get-started"
